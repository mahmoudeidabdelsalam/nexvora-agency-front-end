      ...
      <div className="mx-auto max-w-7xl flex flex-col lg:flex-row gap-16">
        <div className="sticky top-30 h-fit lg:w-2/3">
          ...
          {image.node.sourceUrl && (
            <div className="mt-8">
              <ScrollReveal animation="fadeInUp" delay={180}>
                <img
        ...
        <div className="flex flex-col items-start justify-center gap-8 text-left pt-30 last:mb-0 lg:w-1/3">
          {coreValues.map((item, index) => (
            <ScrollReveal key={item.valuee} animation="fadeInLeft" delay={index * 120}>
              <div 
                className="group flex flex-col items-start justify-center gap-2 text-left"
              >
              {item.icon.node.sourceUrl && (
                  <div className="rounded-md border flex items-center justify-center border-[#0b97ab]/10 bg-[#0b97ab] h-18 w-18 p-3 backdrop-blur opacity-80 transition-opacity group-hover:opacity-100">
                      <img
                          key={item.valuee}
              ...
              <p className="m-0 font-bold text-xl text-[#262263] group-hover:text-[#0997AA] transition-colors">{item.valuee}</p>
                {item.text && (
